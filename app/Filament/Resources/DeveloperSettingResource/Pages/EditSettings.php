...
class EditSettings extends Page implements HasForms
{
    ...
    public static function getTabs()
    {
        ...
        $groups = GeneralSetting::select('group')->groupBy('group')->where('is_active', 0)->get();
    ...
    public static function getFields($group)
    {
        ...
        $group_fields = GeneralSetting::where('group', $group)->where('is_active', 0)->get();
        ...
        foreach ($group_fields as $fields) {
            if ($fields->slug != 'SITE_STATUS') {
                ...
                $data[] = Select::make($fields->slug)->label(ucwords($fields->title))->options(config('constants.siteStatus'))->default($fields->value);
            }
        }
        return $data;
    }
